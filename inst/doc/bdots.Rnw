...
the error correlation assumption (\texttt{cor = FALSE}). We'll first try this using
the newer matrix input (at this point requires the input of parameters) and then with
the more traditional refitting approach.
...
A summary of the quality of fits (the same summary setup as at the end of the fitting methods) can be
output after doing refittings using the \texttt{printFits} method.
