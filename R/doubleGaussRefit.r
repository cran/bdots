		...
		if(group[i] == 1) {
			...
			if(diffs && curves[i] == 2) {
				...
				if(is.null(fit.curve$fit)) {
					...
					sdev.id3.cov[[subj[i]]] <- matrix(NA, nrow = 6, ncol = 6)
					sigma.id3[subj[i],] <- NA
					cor.3[subj[i]] <- NA
					R2.g1.2[subj[i]] <- NA
					fail <- fail + 1
				...
				if(is.null(fit.curve$fit)) {
					...
					sdev.id1.cov[[subj[i]]] <- matrix(NA, nrow = 6, ncol = 6)
					sigma.id1[subj[i],] <- NA
					cor.1[subj[i]] <- NA
					R2.g1.1[subj[i]] <- NA
					fail <- fail + 1
			...
			if(diffs && curves[i] == 2) {
				...
				if(is.null(fit.curve$fit)) {
					...
					sdev.id4.cov[[subj[i]]] <- matrix(NA, nrow = 6, ncol = 6)
					sigma.id4[subj[i],] <- NA
					cor.4[subj[i]] <- NA
					R2.g2.2[subj[i]] <- NA
					fail <- fail + 1
				...
				if(is.null(fit.curve$fit)) {
					...
					sdev.id2.cov[[subj[i]]] <- matrix(NA, nrow = 6, ncol = 6)
					sigma.id2[subj[i],] <- NA
					cor.2[subj[i]] <- NA
					R2.g2.1[subj[i]] <- NA
					fail <- fail + 1
